...
// ###########################################################
//
//      ãƒžã‚¸ãƒ§ã‚«ã‚¢ã‚¤ãƒªã‚¹ 640*48px ç´°é•·LCD å‹•ä½œãƒ†ã‚¹ãƒˆã‚³ãƒ¼ãƒ‰
//
...
//
//      Pinout modified for Adafruit ESP32 Feather 
//      Huzzah + Sushi FeatherWing
//        -- @bikerglen
//
//
// ####################  LCD Pin Assignments  ####################
...
//  22  - GND   [to GND]
//
// â€» PSRAMã‚’ä½¿ç”¨ã™ã‚‹å ´åˆã¯ GPIO 16 , 17 ã¯ä½¿ç”¨ã§ããªã„ã®ã§æ³¨æ„ã€‚

#include <LovyanGFX.hpp>


struct LGFX_Config {
  static constexpr int gpio_wr  = 32; //to LCD WR(14)
  static constexpr int gpio_rd  = 33; //to LCD RD(13)
  static constexpr int gpio_rs  = 14; //to LCD DC(16)
  static constexpr int gpio_d0  = 21; //to LCD D0(4)
  static constexpr int gpio_d1  = 17; //to LCD D1(5)
  static constexpr int gpio_d2  = 16; //to LCD D2(6)
  static constexpr int gpio_d3  = 19; //to LCD D3(7)
  static constexpr int gpio_d4  = 18; //to LCD D4(8)
  static constexpr int gpio_d5  =  5; //to LCD D5(9)
  static constexpr int gpio_d6  =  4; //to LCD D6(10)
  static constexpr int gpio_d7  = 12; //to LCD D7(11)
  // gpio_cs(12/15) = 27
  // gpio_rst(2)    = 13
};
...
// ===== ENTER SUSHI SHOP ===== //
void setup() {
// è¨­å®šã§ãã‚‹ã‚¯ãƒ­ãƒƒã‚¯ã¯20MHzï½ž6.7MHzã§ã™ã€‚ã¾ãŸã€80MHzã‚’æ•´æ•°ã§å‰²ã£ãŸå€¤ã«èª¿æ•´ã•ã‚Œã¾ã™ã€‚
// 20MHz, 16MHz, 13.4MHz, 11.5MHz, 10MHz, 88.9MHz, 8MHz, 7.3MHz, 6.7MHz
  panel.freq_write = 16000000;
  panel.len_dummy_read_pixel = 8;

  panel.spi_cs = 27;  //to LCD CS(15)
  panel.spi_dc = -1;
  panel.gpio_rst = 13;  //to LCD RST(2)
